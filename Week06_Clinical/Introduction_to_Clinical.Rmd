...
```{r}
write.csv(clinic, "/home1/emmashih/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/emmashih/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
# 1. How many rows are there? How many columns? 
      There are 1097 rows and 112 variables.
...
# 3. What is the age in years of the 5th patient (barcode: TCGA-4H-AAAK)?
      The 5th patient is roughly 50 years old.
# 4. How old was the 1st patient at their last follow up (barcode: TCGA-3C-AAAU)? HINT: you need to look at data from two columns to answer this
      The 1st patient was roughly 45 years old since their last followup.

```{r Exercise 3.1}
# write code here
head(clinic)
str(clinic)
clinic["", "birth_days_to"]
18371/365
clinic[1, c("birth_days_to", "last_contact_days_to")]
(20211-3767)/365
```
...
```{r Exercise 4.1}
# create plot here
plot(clinic$age_at_diagnosis, clinic$initial_pathologic_dx_year, xlab="age at diagnosis", ylab="Inititial pathologic year", col = "blue")
```
...
```{r Exercise 4.2}
# create plot here
lymph <- as.numeric(clinic$lymph_nodes_examined_count)
tumor <- as.factor(clinic$tumor_status)
boxplot(lymph ~ tumor, data = clinic)
```
...
```{r}
cleaned_clinic$age_category <- ifelse(clinic$age_at_diagnosis <= 35, "Young", ifelse(cleaned_clinic$age_at_diagnosis > 35 & cleaned_clinic$age_at_diagnosis < 50, "Middle", "Old"))
```
...
# 1. What does the y axis represent?
    In the KM plot, the y axis represents survival probability of the patients.
# 2. What does the x axis represent?
    The x axis represents the time in days after diagnosis of breast cancer in patients.
# 3. What age group appears to have the greatest rate of survival?
    According to the KM plot, the middle age group has the greatest rate of survival.

# 4. What age group appears to have the worst rate of survival? 
    The "Young" age group appears to have the worst rate of survival.
    
# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
    15 years = 15 × 365 = 5,475 days
    Looking at the ~5500 days mark of the graph, "Old" breast cancer patients appear to have a survival rate of around 0.35.
...
# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and breast cancer survival? 
    The p value is 0.2. At a significant threshold of 0.05, a p value of 0.2 is not statistically significant. This tells us that despite each line appearing different, there are no sufficient evidence that the differences in survival rates between age groups are statistically significant. The differences could be due to random variation.
